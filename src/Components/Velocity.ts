...
import { isEqualVectors, TVector, widthVector, zeroVector } from '../utils/shape';
...
export const createVelocityComponent = (v = 0) => createComponent(VelocityComponentID, { v });

export function setVelocity(struct: ExtractStruct<VelocityComponent>, v: number): void {
    struct.v = v;
}
...
export function setVelocityByVector(
    ...
    struct.v = isEqualVectors(direction, zeroVector)
        ? 0
        : widthVector(direction) / (sign(abs(direction.x)) + sign(abs(direction.y)));
}
